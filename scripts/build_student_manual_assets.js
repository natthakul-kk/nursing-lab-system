...
const flowcharts = [
  ...
  },
  {
    ...
    HumanPath[/"🧑‍⚕️ ใช้งานกับคนจริง หรือคลินิก Clinical Patient"/]:::inputOutput
    ...
    CheckUnexpired -->|ไม่พอ หรือหมดอายุ| RejectHuman
    RejectHuman -.->|ปรับลดจำนวน หรือเปลี่ยนรายการ| SelectItems
    ...
    RejectSim -.->|ปรับลดจำนวน หรือเปลี่ยนรายการ| SelectItems
    ...
    RejectEq -.->|เปลี่ยนรายการ หรือเปลี่ยนวันนัดหมาย| SelectItems
    ...
    RejectOfficer["❌ ไม่อนุมัติ: ระบุเหตุผลในระบบ เช่น เกินโควตา หรือไม่สอดคล้อง"]:::warnProcess
    ...
    CheckCondition -->|สมบูรณ์| ReturnOk
    ReturnIssue["⚠️ บันทึกอาการชำรุด หรือส่งซ่อมบำรุง RETURNED_WITH_ISSUE"]:::warnProcess
    CheckCondition -->|ชำรุด หรือมีปัญหา| ReturnIssue
...
async function renderFlowcharts() {
  ...
  for (const fc of flowcharts) {
    ...
    // Verify no syntax error was rendered
    const hasSyntaxError = await page.evaluate(() => {
      const text = document.body.innerText || '';
      return text.includes('Syntax error') || !!document.querySelector('.error-icon') || !!document.querySelector('[id*="dmermaid"]');
    });
    if (hasSyntaxError) {
      throw new Error(`CRITICAL: Mermaid rendered a Syntax Error for flowchart ${fc.id}! Aborting.`);
    }
