    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <BriefcaseMedical className="w-6 h-6 text-teal-600 dark:text-teal-400" />
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition cursor-pointer"
            title="รีเฟรชข้อมูล"
          >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-3 items-center justify-between">
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
          ...
          {['หัตถการพื้นฐาน', 'การฉีดยาและให้สารน้ำ', 'การทำแผลและผ่าตัดเล็ก', 'สูติ-นรีเวชกรรม', 'การพยาบาลเด็ก', 'ทักษะช่วยฟื้นคืนชีพ (CPR)'].map((cat) => (
            <button
              key={cat}
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-12 text-center space-y-3">
          <BriefcaseMedical className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto" />
          <h3 className="text-base font-bold text-slate-700 dark:text-slate-200">ยังไม่มีชุดฝึกปฏิบัติการในหมวดหมู่นี้</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredKits.map((kit) => {
            ...
            return (
              <div
                key={kit.id}
                className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 hover:border-teal-500/40 shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden group"
              >
                {/* Card Header */}
                <div className="p-5 pb-3 border-b border-slate-100 dark:border-slate-800 flex-1 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="font-mono text-[10px] font-bold text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/70 px-2 py-0.5 rounded border border-teal-200/60 dark:border-teal-800">
                        {kit.code}
                      </span>
                      <h3 className="font-bold text-slate-900 dark:text-white text-sm mt-1.5 leading-snug group-hover:text-teal-600 dark:group-hover:text-teal-400 transition">
                        {kit.name}
                      </h3>
                    </div>
                    ...
                    {isStaff && (
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 p-1.5 rounded-lg hover:bg-teal-50 dark:hover:bg-slate-800 transition cursor-pointer"
                        ...
                        <button
                          ...
                          className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-slate-800 transition cursor-pointer"
                          title="ลบชุดฝึกนี้"
                        >
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    ...
                    {kit.targetCourse && (
                      <span className="px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-900/50 font-medium flex items-center gap-1">
                  ...
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1.5">
                    <div className="text-[11px] font-bold text-slate-700 dark:text-slate-300 flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-slate-400 dark:text-slate-500 font-normal">จำนวนต่อ 1 ชุด</span>
                    </div>
                    ...
                    <div className="space-y-1.5 max-h-44 overflow-y-auto pr-1 text-xs">
                      {kit.components?.map((c: any) => {
                        ...
                        return (
                          <div
                            key={c.id}
                            className="flex items-center justify-between text-[11px] py-1.5 px-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-100/80 dark:border-slate-700/60 hover:bg-slate-100/80 dark:hover:bg-slate-700/60 transition"
                          >
                            <div className="flex items-center gap-1.5 truncate max-w-[210px]">
                              {isRepack ? (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-purple-50 dark:bg-purple-950/70 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 flex-shrink-0">
                                  Repack
                                </span>
                              ) : c.type === 'EQUIPMENT' ? (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-950/70 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 flex-shrink-0">
                                  ครุภัณฑ์
                                </span>
                              ) : (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-teal-50 dark:bg-teal-950/70 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-800 flex-shrink-0">
                                  สิ้นเปลือง
                                </span>
                              )}
                              <span className="text-slate-700 dark:text-slate-200 font-medium truncate" title={c.name}>
                                {c.name}
                              </span>
                            </div>

                            <div className="flex items-center gap-2 text-right">
                              <span className="text-slate-700 dark:text-slate-200 font-bold font-mono text-[11px]">
                ...
                <div className="p-4 bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                  <div>
                    <div className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">สต็อกพร้อมจัดได้</div>
                    <div
                      className={`text-sm font-black flex items-center gap-1.5 ${
                        isAvailable ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                      }`}
                    >
                      {isAvailable ? (
                        <>
                  ...
                  <label className="flex items-start gap-2 cursor-pointer text-slate-700">
                    ...
                    <div>
                      <span className="font-bold text-purple-900 flex items-center gap-1">
                        🎁 แถมเศษที่เหลือให้นิสิตไปด้วยเลย (เคลียร์ซองเปิด)
                      </span>
                      <p className="text-[11px] text-slate-500">
                        หากเปิดซองแล้วมีเศษเหลือ หรือมีเศษเหลือน้อย จะแถมให้นิสิตไปทั้งหมด สต็อกเศษจะถูกตัดเป็น 0 ตู้สต็อกสะอาด
                      </p>
                    </div>
                  </label>
                </div>
