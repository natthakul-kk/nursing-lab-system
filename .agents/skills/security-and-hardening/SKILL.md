...
## PDPA & Data Visibility Classification Standards (Class A / Class B / Class C)

Every feature, API query, and UI component MUST strictly enforce data visibility boundaries based on data classification:

1. **Class A: Strictly Confidential (ข้อมูลส่วนบุคคลชั้นความลับสูง - PDPA Sensitive)**
   - *Data:* Leave balances, medical reasons, salary, appraisal scores, private employee history.
   - *Scoping Rule:* Standard users MUST ONLY query their own records (`user_id = auth.uid()`). Never expose query endpoints that list all employee leaves to standard users.
   - *Approver Scoping:* Approvers only view requests routed directly to their turn in `chain_snapshot`.
   - *AI Sanitization:* Strip all PII (names, citizen IDs, phone numbers) before sending metrics to external AI models.

2. **Class B: Shared Operational Resources (ข้อมูลการใช้ทรัพยากรส่วนกลาง)**
   - *Data:* Room booking schedules, academic timetables, vehicle schedules, equipment availability.
   - *Scoping Rule:* Calendar availability is visible to authenticated users to prevent overlaps, but private phone numbers and confidential notes MUST be masked from non-admins.

3. **Class C: Collaborative Project & Workflow (การทำงานร่วมกันระดับฝ่ายงาน)**
   - *Data:* Department task boards, collaborative project tracking.
   - *Scoping Rule:* Accessible to department members and assignees.
