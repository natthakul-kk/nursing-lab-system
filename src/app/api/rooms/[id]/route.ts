...
export async function PUT(
  ...
  try {
    ...
    const { code, name, location, capacity, description, isActive, closeReason } = body;
    ...
    const room = await prisma.practiceRoom.update({
      ...
      data: {
        ...
        ...(closeReason !== undefined && { closeReason: closeReason || null }),
      },
    });
...
export async function DELETE(
  ...
  try {
    ...
    const bookingsCount = await prisma.roomBooking.count({
      where: { roomId: id },
    });
    ...
    if (bookingsCount > 0) {
      await prisma.practiceRoom.update({
        where: { id },
        data: { isActive: false, closeReason: 'ปิดการใช้งานถาวรเนื่องจากมีประวัติการจอง' },
      });
      invalidateCache('practice:rooms:');
      return NextResponse.json({ message: 'ปิดการใช้งานห้องเรียบร้อยแล้ว เนื่องจากมีประวัติการจองห้อง' });
    }
